...
// Remove ALL fake laptop products
const cleanedProducts = products.filter(p => {
  const titleLower = p.title.toLowerCase();
  const isLaptop = titleLower.includes('macbook') || titleLower.includes('laptop');
  
  if (!isLaptop) return true; // Keep non-laptop products
  
  // Remove fake laptops with:
  // - 99% or 100% discount
  // - Prices like ₹99, ₹499, ₹999
  // - Missing images
  const isFake = p.discount >= 99 || 
                 p.price < 10000 || 
                 (p.image && p.image.startsWith('/images/products/macbook'));
  
  if (isFake) {
    console.log(`Removing fake: ${p.title} (₹${p.price})`);
    return false;
  }
...
console.log(`Removed ${products.length - cleanedProducts.length} fake laptop products`);

// Remove duplicates by ID
const seenIds = new Set();
const uniqueProducts = cleanedProducts.filter(p => {
  if (seenIds.has(p.id)) {
    console.log(`Removing duplicate ID: ${p.id} - ${p.title}`);
    return false;
  }
  seenIds.add(p.id);
  return true;
...
console.log(`Removed ${cleanedProducts.length - uniqueProducts.length} duplicate products`);

// Now add real laptops with unique IDs
const realLaptops = [
  {
    id: 'laptop-macbook-air-m3-13',
  ...
  {
    id: 'laptop-lenovo-ideapad-slim-3',
    title: 'Lenovo IdeaPad Slim 3 Intel Core i5 12th Gen (16GB/512GB SSD/Windows 11)',
    price: 48990,
    originalPrice: 69990,
    discount: 30,
    image: 'https://rukminim2.flixcart.com/image/312/312/xif0q/computer/j/z/4/-original-imagfdf4xnbwdfz2.jpeg?q=70',
    rating: 4.2,
    ratingCount: '8,234',
    reviewCount: '1,456',
    category: 'electronics',
    brand: 'Lenovo',
    seller: 'Lenovo Exclusive Store',
    highlights: [
    ...
    description: 'Lenovo IdeaPad Slim 3 offers great value with solid performance. The 12th Gen Intel processor handles everyday tasks smoothly. Slim design makes it perfect for students and professionals on the go.',
    images: [
      'https://rukminim2.flixcart.com/image/416/416/xif0q/computer/j/z/4/-original-imagfdf4xnbwdfz2.jpeg?q=70',
    ],
    specs: {
      'Brand': 'Lenovo',
    ...
    offers: [
      'Bank Offer: 5% Cashback on Flipkart Axis Bank Card',
      'No Cost EMI: Starting from ₹4,083/month',
      'Exchange Offer: Up to ₹8,000 off on old laptop'
    ],
    reviews: []
  }
];
...
// Add real laptops
const finalProducts = [...uniqueProducts, ...realLaptops];
